...
* Talos' IOC list separates 22 "likely actor-controlled" IPs from 38 "multi-tenant" IPs and warns that some addresses are shared or anonymization infrastructure rather than attacker-owned. This report's appendix does not record that distinction; the lab's CDB lists do (see rules 100020–100023).
...
2. **Convert the IOC list into a SIEM lookup.** *(Implemented.)* The 60 IPv4 indicators were loaded as two Wazuh CDB lists, split by Talos' own classification (22 likely actor-controlled, 38 multi-tenant), and matched against pfSense firewall logs by custom rules 100020/100021 (actor-controlled tier, level 12) and 100022/100023 (multi-tenant tier, level 8), each covering source and destination matches. This is the first case in this lab of CTI feeding detection rather than sitting alongside it. Note that this is forward-looking alerting, not retrospective hunting: it only matches traffic logged after the pipeline was built. See See [Rules 100020–100023](../detection-rules/100020-100023-arcanedoor-cdb-correlation.md).
...
* **CDB matching is alerting, not blocking.** In this lab the lists drive alerts for triage only (level 12 for the likely actor-controlled tier, level 8 for the multi-tenant tier). Because the indicators are more than two years old, treat a hit as a lead that needs enrichment, expect false positives from reassigned addresses, and refresh or retire the list on a documented schedule.
